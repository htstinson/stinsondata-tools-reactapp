...
const UserGrid = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      let url = 'https://stinsondata.tools/api/v1/users';
  ...
  const handleSubmit = async (user) => {
    try {
      ...
      const url = user.id 
        ? `https://stinsondata.tools/api/v1/users/${user.id}`
        : 'https://stinsondata.tools/api/v1/users';
  ...
  const handleDelete = async (dataItem) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://stinsondata.tools/api/v1/users/${dataItem.id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
